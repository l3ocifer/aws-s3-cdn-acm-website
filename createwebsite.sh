...
update_repo_from_template() {
    ...
    # Update specific files with customized content without creating -e files
    ...
    # Clean up any potential backup files created by sed (in case sed creates them)
    rm -f scripts/setup_terraform.sh-e terraform/backend.tf-e

    # Commit the changes
    git add .
    git commit -m "Update from template repository" || true  # Commit only if there are changes
